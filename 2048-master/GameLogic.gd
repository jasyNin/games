...
var _blocks = []
var _previous_states = []  # Массив для хранения предыдущих состояний
...
func _input(event):
	...
	if event.is_action_pressed("ui_undo"):  # Добавить обработку нажатия кнопки UNDO
		_undo_last_action()
...
func _save_current_state():
	...
	for block in _blocks:
		if block:
			state.append(block.get_type())  # Сохраняем тип блока
		else:
			state.append(null)  # Если блока нет, добавляем null
	_previous_states.append(state)

func _undo_last_action():
	if _previous_states.size() > 0:
		var last_state = _previous_states.pop_back()  # Получаем последнее сохраненное состояние
		for i in range(_size * _size):
			if last_state[i]:  # Проверяем, есть ли блок
				var block = Block.instantiate()
				block.position = _calc_screen_position(i)
				block.set_type(last_state[i])
				block.connect("point_scored", Callable(GUI, "on_point_scored"))
				$Pivot.add_child(block)
				_blocks[i] = block
			else:
				_blocks[i] = null  # Если блока не было, устанавливаем null
